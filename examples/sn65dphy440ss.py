...
This is a self-contained copy of the component model used by the adjacent
electrical monorepo. The monorepo-specific KiCad footprint and fixed placement
are intentionally omitted so the example can run from a standalone Earthground
checkout. It demonstrates ratings, configuration straps, required-external
contracts, power estimation, and thermal metadata.
"""
...
if __name__ == "__main__":
    ...
    for result in example.check_straps().results:
        print(
            f"  {result.refdes}.{result.pin}: {result.status.value} - {result.message}"
        )
